...
# Streamlit Application Specification: AI Governance Framework and Ethical Checklist

## 1. Application Overview

**Purpose of the Application**

The "QuantAlpha AI Governance & Ethical Checklist" application is designed for financial professionals, specifically CFA Charterholders and Investment Professionals, to systematically evaluate AI models against an ethical framework and a robust governance policy. The application guides users through understanding and applying a five-pillar AI governance framework, classifying models by risk, applying an ethical checklist, defining human oversight, mapping regulatory compliance, and compiling a formal AI Governance Policy Document. Its core purpose is to facilitate the responsible and ethical deployment of AI in financial services, ensuring compliance and building client trust.

**High-level Story Flow of the Application**

The application simulates the workflow of Alex Chen, a Senior Risk Analyst at QuantAlpha Investments. Alex begins by reviewing QuantAlpha's foundational **Five-Pillar AI Governance Framework**. He then uses a **Model Risk Tiering System** to classify various AI models based on their risk profile, helping to allocate governance resources effectively. Next, he applies a **10-question Ethical Checklist** to specific models, assessing their alignment with FATPSR principles and identifying governance gaps. Alex then defines appropriate **Human Oversight Policies** for different AI decision types and maps internal controls to **Regulatory Compliance** requirements (e.g., SR 11-7, EU AI Act, ECOA, FINRA). Finally, he compiles all these elements into a formal **AI Governance Policy Document**, a critical deliverable for QuantAlpha's AI Governance Committee. Throughout this journey, the application emphasizes real-world application, workshop-style discussion, and the generation of actionable insights for responsible AI deployment.

## 2. Code Requirements

### Import Statement

The application will begin by importing all necessary components from `source.py`:
...
from source import * # Imports all functions, ETHICAL_CHECKLIST, OVERSIGHT_POLICY, REGULATORY_MAP, course_models, tier_df
```
...
if 'current_page' not in st.session_state:
    st.session_state.current_page = 'Home' # Current page selected in the sidebar

# --- Static Data from source.py (loaded once) ---
if 'course_models_data' not in st.session_state:
    st.session_state.course_models_data = course_models # List of dictionaries from source.py
if 'tier_df_data' not in st.session_state:
    st.session_state.tier_df_data = tier_df # Pandas DataFrame from source.py
if 'ethical_checklist_questions' not in st.session_state:
    st.session_state.ethical_checklist_questions = ETHICAL_CHECKLIST # List of dictionaries from source.py
if 'oversight_policy_data' not in st.session_state:
    st.session_state.oversight_policy_data = OVERSIGHT_POLICY # Dictionary from source.py
if 'regulatory_map_data' not in st.session_state:
    st.session_state.regulatory_map_data = REGULATORY_MAP # Dictionary from source.py
if 'governance_policy_document_content' not in st.session_state:
    # Call the function from source.py once and store its output
...
if 'selected_checklist_model_name' not in st.session_state:
    # Initialize with the first model's name for convenience
...
if 'user_checklist_answers' not in st.session_state:
    # Stores user answers for ethical checklist per model: {'Model Name': {question_id: 'yes'|'no'|'partial'}}
    st.session_state.user_checklist_answers = {model['model']: {} for model in st.session_state.course_models_data}
    # Pre-populate with example answers for Credit Default XGBoost and Trading RL Agent from source.py for demonstration
...
if 'ethical_evaluation_results' not in st.session_state:
    # Stores the results returned by apply_checklist, plus pillar-wise scores for radar chart
    # Format: {'Model Name': {'score': float, 'grade': str, 'gaps': int, 'pillar_scores': {pillar: score_pct}}}
    st.session_state.ethical_evaluation_results = {}

# --- Helper function (Streamlit display logic) for Pillar-wise Score Calculation ---
def calculate_pillar_scores_for_display(model_name, user_answers_for_model, ethical_checklist_data):
    ...
    for q_item in ethical_checklist_data:
        ...
        answer = user_answers_for_model.get(q_id, 'no') # Default to 'no' if not answered
...
# Pre-run ethical checklist for the example models to have initial results
for model_name, answers in st.session_state.user_checklist_answers.items():
    if answers: # Only if answers are pre-populated
        result = apply_checklist(model_name, answers) # Call function from source.py
        pillar_scores = calculate_pillar_scores_for_display(model_name, answers, st.session_state.ethical_checklist_questions)
        st.session_state.ethical_evaluation_results[model_name] = {
            'score': result['score'],
            'grade': result['grade'],
            'gaps': result['gaps'],
...
*   **`st.session_state.current_page`**: Updated by the sidebar selectbox (`st.sidebar.selectbox`). Read by the main content area to conditionally render pages.
*   **`st.session_state.selected_checklist_model_name`**: Updated by a `st.selectbox` on the "Ethical Checklist" page. Read to filter which model's questions and answers are displayed.
*   **`st.session_state.user_checklist_answers`**: Updated by `st.radio` widgets on the "Ethical Checklist" page. Read when `apply_checklist` is invoked and to display current answers.
*   **`st.session_state.ethical_evaluation_results`**: Updated after `apply_checklist` is invoked (including the helper `calculate_pillar_scores_for_display`). Read to display scores, grades, gaps, and radar charts on the "Ethical Checklist" and "Final Report" pages.
*   **Static data (`course_models_data`, `tier_df_data`, etc.)**: Loaded once at initialization and read directly on their respective display pages.
...
```python
    ...
    """)
    # Visualization: Conceptual Diagram (textual for now as no image provided)
...
```python
    ...
    selected_model_name = st.selectbox(
        ...
        index=model_options.index(st.session_state.selected_checklist_model_name) if st.session_state.selected_checklist_model_name in model_options else 0,
    ...
    if answers_updated:
        st.experimental_rerun() # Rerun to update the results based on new answer
    ...
    if st.button(f"Generate Evaluation Report for {selected_model_name}"):
        ...
        raw_result = apply_checklist(selected_model_name, answers_for_model) # This function also prints to console
        ...
        st.session_state.ethical_evaluation_results[selected_model_name] = {
            ...
            'pillar_scores': pillar_scores_for_display # Store pillar scores for visualization
        }
    ...
    if selected_model_name in st.session_state.ethical_evaluation_results:
        ...
        # Radar Chart for Pillar Scores (Visualization Requirement V3)
